...
# Instructions
instructions = tk.Label(root, text="Select a File to extract it's text", font="Raleway")
instructions.grid(columnspan=3, column=0, row=1)


def open_file():
    browse_text.set("loading...")
    file = askopenfilename(parent=root, title="Choose a file", filetypes=[("PDF Files", "*.*")])
    if file:
